      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content border-0" style={{ borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ background: 'linear-gradient(135deg,#0B174E,#25233F)', padding: '1.2rem 1.5rem' }}>
            <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.72rem', letterSpacing: 1 }}>ADMIN</div>
            <div style={{ color: '#fff', fontWeight: 800, fontSize: '1.05rem' }}>
              <i className="bi bi-gear-fill me-2" />Payment Configuration
            </div>
          </div>
          ...
          <div className="modal-body p-4">
            ...
            <div className="row g-3">
              <div className="col-6">
                <label className="form-label small fw-semibold">Default Amount</label>
                <input
            ...
            <div className="mt-3 p-3 rounded" style={{ background: '#FEF3C7', fontSize: '0.78rem', color: '#92400E' }}>
              <i className="bi bi-info-circle me-1" />
              <strong>Note:</strong> If "Require Admin Verification" is OFF, student payments are auto-verified on "Pay Now". If ON, you must manually verify each one below.
            </div>
          </div>
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content border-0" style={{ borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ background: 'linear-gradient(135deg,#0B174E,#25233F)', padding: '1.2rem 1.5rem' }}>
            ...
            <div style={{ color: '#fff', fontWeight: 800, fontSize: '1.05rem' }}>Fee Receipt — {payment.month} {payment.year}</div>
          </div>
          <div className="modal-body p-4" style={{ position: 'relative' }}>
            {payment.status === 'Verified' && (
              <div style={{ position: 'absolute', top: 16, right: 20, border: '3px solid #22C55E', borderRadius: 8, padding: '3px 12px', color: '#22C55E', fontWeight: 800, fontSize: '1rem', transform: 'rotate(15deg)', opacity: 0.8, letterSpacing: 2 }}>PAID</div>
            )}
            ...
            <DRow label="Payment Method"  value="Demo Payment" />
...
function DRow({ label, value, mono }) {
  return (
    <div className="d-flex justify-content-between mb-2">
      ...
      <span style={{ fontSize: '0.8rem', fontWeight: 600, maxWidth: '60%', textAlign: 'right', wordBreak: 'break-all', fontFamily: mono ? 'monospace' : 'inherit' }}>{value}</span>
    </div>
  )
}
...
export default function PaymentAdmin() {
  ...
  const [actionNote, setActionNote] = useState({}) // paymentId → note
  const [acting, setActing]         = useState(null) // paymentId being acted on
  ...
  const handleVerify = async (id) => {
    setActing(id)
    try {
      await verifyPayment(id, { adminNote: actionNote[id] || '' })
  ...
  const handleReject = async (id) => {
    setActing(id)
    try {
      await rejectPayment(id, { adminNote: actionNote[id] || '' })
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">View all student payments, update fee settings, and verify transactions.</p>
      ...
      {/* ── Config Banner ── */}
      {config && (
        <div className="card mb-4" style={{ border: '1.5px dashed #0B174E', background: 'rgba(11,23,78,0.03)' }}>
          <div className="card-body d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
              <div style={{ fontWeight: 700, color: '#0B174E' }}>
                <i className="bi bi-gear me-2" />Fee Configuration
              </div>
              <div className="text-muted" style={{ fontSize: '0.82rem' }}>
                Default: <strong>{config.currency} {(config.defaultAmount || 0).toLocaleString('en-IN')}</strong>
                &nbsp;·&nbsp; Due: <strong>Day {config.dueDay}</strong>
                &nbsp;·&nbsp; Auto-verify: <strong>{config.requiresVerification ? 'OFF (manual)' : 'ON'}</strong>
                &nbsp;·&nbsp; Total Revenue: <strong style={{ color: '#22C55E' }}>{config.currency} {revenue.toLocaleString('en-IN')}</strong>
              </div>
            </div>
            <button className="btn btn-outline-primary btn-sm" style={{ borderRadius: 8 }} onClick={() => setShowConfig(true)}>
              <i className="bi bi-pencil me-1" />Edit Config
            </button>
          </div>
        </div>
      )}
          ...
          <div className="row g-2 align-items-center">
            ...
            <div className="col-md-4">
              <select className="form-select form-select-sm" value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                ...
                <option value="Uploaded">Uploaded (Awaiting Verify)</option>
                  ...
                  filtered.map(p => {
                    ...
                    return (
                      <tr key={p._id}>
                        ...
                        <td className="small">{p.month} {p.year}</td>
                        <td className="small fw-semibold">{p.currency || 'INR'} {(p.amount || 0).toLocaleString('en-IN')}</td>
                        ...
                        <td>
                          <div className="d-flex gap-1 flex-wrap align-items-center">
                            ...
                            {/* Verify (for Pending or Uploaded) */}
                            ...
                            {/* Reject (for Pending, Uploaded, or Verified) */}
                            {p.status !== 'Rejected' && (
                              <button
      ...
      {viewPayment && (
        <DetailModal payment={viewPayment} onClose={() => setViewPayment(null)} />
      )}
    </div>
  )
}
